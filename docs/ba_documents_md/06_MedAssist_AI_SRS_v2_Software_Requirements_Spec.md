...
4. Yêu cầu chức năng (Functional Requirements) 4
   ...
   4.6 UC-05: Thu thập thông tin bệnh nhân & Tạo Form Đăng ký (Info Collection + Pre-filled Form) 7
   ...
   4.12 UC-12: Nhập liệu bằng giọng nói — Speech-to-Text (Accessibility) 9
...
12. Phụ lục 14
    ...
    12.3 Ưu tiên tính năng (Workshop 2 & 3) 15

---

## Lịch sử thay đổi (Change Log)

| Version | Date       | Mô tả thay đổi                                                                 |
|---------|------------|---------------------------------------------------------------------------------|
| 1.0     | 2026-04-18 | Bản SRS gốc — Workshop 2 scope (UC-00 → UC-04)                                |
| 2.0     | 2026-04-22 | Workshop 3: Cập nhật UC-05 (pre-fill form), thêm UC-12 (STT), NFR/BR/API mới  |

### Chi tiết thay đổi V1 → V2:
- **UC-05** (chỉnh sửa): Thêm FR-05-03 → FR-05-07 — lưu patient_info vào session dictionary ở BE, gửi link pre-filled registration form, API GET patient-info
- **UC-12** (thêm mới): Speech-to-Text via OpenAI Whisper API — FR-12-01 → FR-12-08
- **API mới**: `POST /api/chat/speech-to-text`, `GET /api/sessions/{session_id}/patient-info`
- **NFR mới**: NFR-PERF-05, NFR-USE-05, NFR-USE-06
- **BR mới**: BR-13 (lưu tạm session dict), BR-14 (pre-fill form)
- **Dependencies mới**: OpenAI Whisper API (whisper-1)
- **Thuật ngữ mới**: STT, Whisper, Pre-fill
- **State Machine**: Thêm node SEND_REGISTRATION_LINK
- **Scope**: Mở rộng Workshop 3
...
1. Giới thiệu
    ...
    Phạm vi Workshop 3 mở rộng: UC-05 (cập nhật), UC-12 (Speech-to-Text).
    ...
    | BHYT | Bảo hiểm y tế |
    | **STT** | **Speech-to-Text — chuyển đổi giọng nói thành văn bản** *(V2 mới)* |
    | **Whisper** | **Model AI của OpenAI chuyên dụng cho speech recognition** *(V2 mới)* |
    | **Pre-fill** | **Tự động điền sẵn dữ liệu vào form từ thông tin đã thu thập qua chat** *(V2 mới)* |
...
2. Mô tả tổng quan hệ thống
    ...
    → SEND_REGISTRATION_LINK → CONFIRMATION → COMPLETED
    ```
...
4. Yêu cầu chức năng (Functional Requirements)
    ...
    ### ⬇️ CÁC PHẦN DƯỚI ĐÂY ĐƯỢC CẬP NHẬT/THÊM MỚI TRONG V2 (Workshop 3) ⬇️

    ---

    4.6 UC-05: Thu thập thông tin bệnh nhân & Tạo Form Đăng ký (Info Collection + Pre-filled Form) *(V2 — cập nhật)*

    > **Thay đổi so với V1:** Thêm FR-05-03 → FR-05-07. Cơ chế mới: lưu patient_info vào session dictionary ở BE, gửi link form pre-fill thay vì auto-fill trực tiếp trong chat.
    ...
    **FR-05-03 — *(V2 mới)* Mỗi trường thông tin sau khi validate thành công phải được lưu ngay vào patient_info dictionary trong session state ở Backend, với key là session_id. Dictionary này cũng bao gồm department_id, department_name, doctor_id, doctor_name đã được xác định từ UC-04.**
    **FR-05-04 — *(V2 mới)* Sau khi thu thập đủ thông tin bắt buộc, chatbot phải tạo và gửi cho bệnh nhân một đường link dạng: `{frontend_base_url}/appointment/register?session_id={session_id}`. Link này mở ra form đăng ký khám bệnh trên Frontend.**
    **FR-05-05 — *(V2 mới)* Khi bệnh nhân mở link form, Frontend gọi API `GET /api/sessions/{session_id}/patient-info` để lấy dữ liệu đã collect và tự động điền (pre-fill) vào tất cả các field, bao gồm:**
    • Thông tin cá nhân (họ tên, ngày sinh, giới tính, SĐT, email, CCCD/BHYT, địa chỉ)
    • Khoa khám (department_name)
    • Bác sĩ gợi ý (doctor_name)
    • Triệu chứng tóm tắt
    • Mức độ nghiêm trọng (severity)
    **FR-05-06 — *(V2 mới)* Bệnh nhân có thể chỉnh sửa thông tin trên form trước khi submit. Submit form sẽ gọi `POST /api/appointments` để tạo lịch hẹn chính thức.**
    **FR-05-07 — *(V2 mới)* Dữ liệu patient_info trong session dictionary phải tự động hết hạn cùng session (24 giờ). Không lưu PII vĩnh viễn trước khi bệnh nhân xác nhận.**
    Postcondition: Bệnh nhân nhận link form đã pre-fill; sẵn sàng review và submit.
    ...
    4.12 UC-12: Nhập liệu bằng giọng nói — Speech-to-Text (Accessibility) *(V2 — thêm mới)*

    > **Mới trong V2:** Toàn bộ UC-12 được thêm mới cho Workshop 3. Hỗ trợ người khiếm thị nhập liệu bằng giọng nói.

    • Actor: Bệnh nhân (đặc biệt người khiếm thị)
    • Trigger: Bệnh nhân nhấn nút microphone trên giao diện chat
    • Precondition: Trình duyệt cho phép truy cập microphone; session đang hoạt động
    • Priority: P0 — Must Have (Workshop 3)

    **FR-12-01** — Giao diện chatbot phải có nút microphone (🎤) bên cạnh ô nhập tin nhắn, cho phép bệnh nhân ghi âm giọng nói thay vì gõ phím.
    **FR-12-02** — Khi bệnh nhân nhấn nút microphone, Frontend phải ghi âm audio từ microphone của thiết bị và gửi file audio tới Backend API để chuyển đổi thành text.
    **FR-12-03** — Backend phải sử dụng OpenAI Whisper API (model whisper-1) để chuyển đổi speech thành text. GPT-4o-mini không hỗ trợ trực tiếp speech-to-text.
    **FR-12-04** — Hệ thống phải ưu tiên hỗ trợ tiếng Việt với độ chính xác chấp nhận được. Whisper API hỗ trợ tiếng Việt (language code: "vi"). Khi gọi API, phải truyền parameter language="vi" nếu session language là tiếng Việt, hoặc để auto-detect cho các ngôn ngữ khác.
    **FR-12-05** — Sau khi nhận kết quả text từ Whisper, text phải được hiển thị trong ô nhập tin nhắn để bệnh nhân xác nhận/chỉnh sửa trước khi gửi.
    **FR-12-06** — Nếu transcription thất bại hoặc không nhận diện được giọng nói, hệ thống phải hiển thị thông báo lỗi thân thiện: "Không nhận diện được giọng nói. Vui lòng thử lại hoặc nhập bằng bàn phím."
    **FR-12-07** — Audio format gửi lên phải là webm/opus hoặc wav, kích thước tối đa 25MB (giới hạn của Whisper API).
    **FR-12-08** — Frontend phải hiển thị trạng thái ghi âm rõ ràng (animation/icon đổi màu) để người dùng biết đang ghi âm.
    Postcondition: Text từ giọng nói được hiển thị trong ô nhập; bệnh nhân xác nhận và gửi như tin nhắn thông thường.
...
5. Yêu cầu phi chức năng (Non-Functional Requirements)
    ...
    | **NFR-PERF-05** | **Speech-to-Text (Whisper) phải trả kết quả trong ≤ 5 giây cho audio ≤ 30 giây** *(V2 mới)* |
    ...
    | **NFR-USE-05** | **Giao diện chatbot phải hỗ trợ nhập liệu bằng giọng nói (STT) cho người khiếm thị, ưu tiên tiếng Việt** *(V2 mới)* |
    | **NFR-USE-06** | **Nút microphone phải có aria-label phù hợp cho screen reader** *(V2 mới)* |
...
6. Yêu cầu giao diện ngoài (External Interface Requirements)
    ...
    • Chat Interface: Giao diện chatbot dạng bubble chat, hỗ trợ SSE streaming (chữ xuất hiện dần), **có nút microphone cho Speech-to-Text** *(V2 mới)*
    • Appointment Form: **Form đăng ký khám bệnh với các field được pre-fill từ thông tin đã thu thập qua chat (mở qua link từ chatbot)** *(V2 cập nhật)*
    ...
    | **Backend ↔ OpenAI Whisper** | **HTTPS (OpenAI API v1) — Speech-to-Text transcription** *(V2 mới)* |
    ...
    **Chat**
    ...
    | **POST** | **/api/chat/speech-to-text** | **Gửi audio file, nhận text (Whisper STT)** *(V2 mới)* |

    **Session Patient Info** *(V2 mới)*

    | Method | Endpoint | Mô tả |
    |--------|----------|-------|
    | **GET** | **/api/sessions/{session_id}/patient-info** | **Lấy thông tin bệnh nhân đã collect (cho pre-fill form)** |
...
7. Quy tắc nghiệp vụ (Business Rules)
    ...
    | **BR-13** | **Thông tin bệnh nhân được collect qua chat phải lưu tạm vào session dictionary (key = session_id), không lưu vĩnh viễn cho đến khi bệnh nhân submit form đăng ký** *(V2 mới)* |
    | **BR-14** | **Form đăng ký khám bệnh phải được pre-fill từ dữ liệu đã collect, bao gồm cả thông tin khoa và bác sĩ gợi ý** *(V2 mới)* |
...
9. Giả định và phụ thuộc (Assumptions & Dependencies)
    ...
    | **OpenAI Whisper API (whisper-1)** | **Speech-to-Text transcription, hỗ trợ tiếng Việt** *(V2 mới)* |
...
10. Phạm vi hệ thống (Scope)
    ...
    | ✅ | **Tự động thu thập thông tin bệnh nhân qua hội thoại, lưu vào session dictionary ở BE** *(V2 cập nhật)* |
    | ✅ | **Gửi link mở form đăng ký khám bệnh với thông tin đã pre-fill (cá nhân + khoa + bác sĩ)** *(V2 mới)* |
    | ✅ | **Speech-to-Text hỗ trợ nhập liệu bằng giọng nói (Whisper API, ưu tiên tiếng Việt)** *(V2 mới)* |
...
12. Phụ lục
    ...
    | **UC-05** | **FR-05-01 → FR-05-07** | **BR-07, BR-08, BR-13, BR-14** | — *(V2 cập nhật)* |
    ...
    | **UC-12** | **FR-12-01 → FR-12-08** | **—** | **—** *(V2 mới)* |
    ...
    │
    ▼
    INFO_COLLECTION ──(đủ info)──► SEND_REGISTRATION_LINK     ← V2 cập nhật
    │
    ▼
    SEND_REGISTRATION_LINK ──► Bệnh nhân mở form pre-fill     ← V2 mới
    │
    ▼
    CONFIRMATION
    │
    ...
    12.3 Ưu tiên tính năng (Workshop 2 & 3)
    ...
    **Workshop 3 Scope:** *(V2 mới)*

    | Ưu tiên | UC | Tính năng |
    |---------|----|-----------|
    | P0 — Must Have | UC-05 (updated) | Info Collection + Pre-filled Registration Form |
    | P0 — Must Have | UC-12 | Speech-to-Text (Whisper API, ưu tiên tiếng Việt) |

---

Tài liệu này tổng hợp và chuẩn hóa các yêu cầu từ BRD, Functional Spec, và Technical Spec theo chuẩn IEEE 830. Mọi thay đổi yêu cầu phải được cập nhật đồng bộ trên tất cả tài liệu liên quan.
