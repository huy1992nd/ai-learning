...
2. **Root Directory** (Vercel → Project → **Settings** → **Build and Deployment** → **Root Directory**):
   ...
   - Cách kiểm nhanh: trên GitHub mở repo → nếu thấy `pyproject.toml` ngay ở gốc thì Root Directory để trống; nếu file nằm ở `src/api/pyproject.toml` thì Root Directory = `src/api`.
