...
7. Kiểm tra config: `curl -s https://<fe-vercel>/assets/app-config.json` — JSON `apiBaseUrl` thường là `/api`. API đi qua [`api/[...path].ts`](./api/[...path].ts) (gửi `ngrok-skip-browser-warning` tới tunnel). Đặt biến Vercel `NGROK_API_ORIGIN` khi URL ngrok đổi.
