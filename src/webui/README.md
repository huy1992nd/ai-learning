...
7. Kiểm tra config: `curl -s https://<fe-vercel>/assets/app-config.json` (hoặc DevTools → Network khi mở app). Phải thấy JSON `apiBaseUrl` ngrok. **Không** mở file JSON trực tiếp trên thanh địa chỉ — rewrite SPA cũ có thể redirect về `/` (đã sửa `vercel.json` chỉ fallback khi `Accept: text/html`).
